...
class FlexibleGroupFilter:
    """
    A class for filtering AnnData objects based on aggregated expression levels of custom groups.
    
    This class provides methods to filter transcripts based on the expression levels
    of their groups (e.g., synteny groups, gene families, pathways) across different samples.
    """
    
    def __init__(
        self,
        default_group_col: str = 'Synt_id',
        default_group_source: Literal['obs', 'var', 'obsm', 'varm'] = 'obsm',
        default_layer: Optional[str] = None,
        verbose: bool = False
    ):
        """
        Initialize the FlexibleGroupFilter.
        
        Parameters
        ----------
        default_group_col : str, default='Synt_id'
            Default column name containing group IDs
        default_group_source : str, default='obsm'
            Default location of the group column in AnnData:
            - 'obs': Use adata.obs[group_col]
            - 'var': Use adata.var[group_col]
            - 'obsm': Use adata.obsm[group_col]
            - 'varm': Use adata.varm[group_col]
        default_layer : str or None, default=None
            Default layer to use for expression values. If None, use .X
        verbose : bool, default=False
            Whether to print additional information during filtering
        """
        self.default_group_col = default_group_col
        self.default_group_source = default_group_source
        self.default_layer = default_layer
        self.verbose = verbose
        self._last_dropped_ids = set()
    ...
    def filter_by_group_expression(
        ...
        layer : str or None, default=None
            Layer to use for expression values. If None, use default_layer
            If both are None, use .X
        group_col : str or None, default=None
            Column name containing group IDs. If None, use default_group_col
        group_source : str or None, default=None
            Location of the group column in AnnData. If None, use default_group_source
        ...
        if group_col is None:
            group_col = self.default_group_col
        if group_source is None:
            group_source = self.default_group_source
        if layer is None:
            layer = self.default_layer
        
        # Get group mapping
        group_mapping = self._get_group_mapping(adata, group_col, group_source)
        ...
        self._last_dropped_ids = dropped_group_ids
        
        # Filter the AnnData object
        if filter_axis == 0:  # Filter rows
            keep_indices = group_mapping.isin(keep_group_ids)
            subset_method = "_inplace_subset_obs" if not copy else None
        else:  # Filter columns
            keep_indices = group_mapping.isin(keep_group_ids)
            subset_method = "_inplace_subset_var" if not copy else None
    ...
    def get_last_dropped_ids(self) -> Set[str]:
        """
...
def filter_low_expression(adata, min_expression=1.0, counts_layer='unique_counts'):
    """
    ...
    """
    filter_obj = FlexibleGroupFilter(default_group_col='Synt_id', 
                                     default_group_source='obsm',
                                     default_layer=counts_layer)
    
    return filter_obj.filter_by_group_expression(adata, min_expression=min_expression)